...
__global__ void summing_gpu(struct DF * device_df, datablockid_t datablock_id){
   ...
   // for (int i = 0; i < COUNT; ++i) {
	...
	printf("%lf ", device_df->ampl);
	printf("%lf\n", (device_df->phi)*180.0/M_PI);
//}
}
...
__global__ void multiply(float *in, float *out){
    ...
    for (int i = 0; i < ARR_SIZE; i++){
        cur_thread_id = i % THREADS_GPU;
        if (tid < THREADS_GPU && cur_thread_id == tid){
            out[i] = in[i] * 2;
        }
    }
}
...
__global__ void phase_rotation_kernel(struct DF * device_df, double curfreq, double curphase, int datablockSize) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < datablockSize) {
        double decodedval = device_df->decoded_data[idx];
        double phi_i = curphase + curfreq * (double)idx;
        phi_i = (phi_i - floor(phi_i)) * 2.0 * M_PI;
        device_df->phase_data[idx].C = decodedval * cos(phi_i);
        device_df->phase_data[idx].S = decodedval * sin(phi_i);
    }
}

__global__ void low_pass_filter_kernel(struct DF * device_df, int const_LPFThreshold, int datablockSize) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < datablockSize) {
        device_df->after_filter_data[idx].S = device_df->after_filter_data[idx].C = 0.0;
        ...
        for (int k = idx; k < lastDataunitIndex; k++) {
            device_df->after_filter_data[idx].C += device_df->phase_data[k].C;
            device_df->after_filter_data[idx].S += device_df->phase_data[k].S;
        }

        device_df->after_filter_data[idx].C /= (double)const_LPFThreshold;
        device_df->after_filter_data[idx].S /= (double)const_LPFThreshold;
    }
}

__global__ void fourfold_frequency_GPU(struct DF * device_df, int size) {
	int tid = blockIdx.x * blockDim.x + threadIdx.x;
	if (tid < size) {
		double re2 = device_df->after_filter_data[tid].C * device_df->after_filter_data[tid].C - device_df->after_filter_data[tid].S * device_df->after_filter_data[tid].S;
		double im2 = 2.0 * device_df->after_filter_data[tid].C * device_df->after_filter_data[tid].S;

		device_df->fourfold_phase_data[tid].C = re2 * re2 - im2 * im2;
		device_df->fourfold_phase_data[tid].S = 2.0 * re2 * im2;
	}
}
...
__device__ double atomicAdd_double(double* address, double val) {
    ...
    } while (old_val != atomicCAS(address_as_ull, old_val, new_val));
    return __longlong_as_double(old_val);
}

__global__ void sum_GPU(struct DF * device_df, int size) {
	int tid = blockIdx.x * blockDim.x + threadIdx.x;
	if (tid < size) {
		atomicAdd_double(&(device_df->Cs), device_df->fourfold_phase_data[tid].C);
		atomicAdd_double(&(device_df->Ss), device_df->fourfold_phase_data[tid].S);
	}
}
...
GPUAnswer gpu_correlate( dataunit_t *origdata, datablockid_t datablockindex, double corrfreq, double corrphase ){
    ...
    float tempdecoded[ POW2 ];
    for (int j = 0; j < 10000; ++j) {
        decoding( origdata[ j ], &tempdecoded[ j ] );
    }
    cudaMemcpy(device_memory_pointer->decoded_data, tempdecoded, 10000 * sizeof( float ), cudaMemcpyHostToDevice);
	...
	int blocksPerGrid = (datablockSize + threadsPerBlock - 1) / threadsPerBlock;
    
    cudaEventRecord(start);

    phase_rotation_kernel<<<blocksPerGrid, threadsPerBlock>>>(device_memory_pointer, corrfreq, corrphase, datablockSize);
    cudaDeviceSynchronize();

    low_pass_filter_kernel<<<blocksPerGrid, threadsPerBlock>>>(device_memory_pointer, 10, datablockSize);
    cudaDeviceSynchronize();

    fourfold_frequency_GPU<<<blocksPerGrid, threadsPerBlock>>>(device_memory_pointer, datablockSize);  
    cudaDeviceSynchronize();

    sum_GPU<<<blocksPerGrid, threadsPerBlock>>>(device_memory_pointer, datablockSize);
    cudaDeviceSynchronize();
    ...
    cudaMemcpy(&reSum, &device_memory_pointer->Cs, sizeof( device_memory_pointer->Cs ) , cudaMemcpyDeviceToHost);
    cudaMemcpy(&imSum, &device_memory_pointer->Ss, sizeof( device_memory_pointer->Ss ) , cudaMemcpyDeviceToHost);
    ...
    answer.ampl = sqrt(reSum * reSum + imSum * imSum) ;
