...
__global__ void phase_rotation_GPU(struct DF * device_df, double freq, datablockid_t datablock_id, float * device_cos_arr, float * device_sin_arr){
    ...
    //for (int j = 0; j < POW2; ++j) {
        ...
        */
       // printf("cos = %f\n", cos_arr[alpha_int]);
        device_df->phase_data[j].C = device_df->decoded_data[j] * device_cos_arr[alpha_int];
        device_df->phase_data[j].S = device_df->decoded_data[j] * device_sin_arr[alpha_int];
...
double gpu_get_phi( double freq, dataunit_t *data,  datablockid_t datablock_id ) {
    ...
    cudaMemcpy(sinus_array, tmp_s_arr, 360 * sizeof( float ), cudaMemcpyHostToDevice);
    cudaMemcpy(cosinus_array, tmp_c_arr, 360 * sizeof( float ), cudaMemcpyHostToDevice);
...
clock_t before = clock();
    phase_rotation_GPU<<<BLOCKS_GPU,1>>>(device_memory_pointer, freq, datablock_id, cosinus_array, sinus_array);
