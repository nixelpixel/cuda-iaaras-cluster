...
double gpu_get_phi( double freq, dataunit_t *data,  datablockid_t datablock_id,  struct DF * device_memory_pointer  ) {
    ...
    printf("%ld\n", sizeof(float) * 10000);
    printf("%ld\n", sizeof(device_memory_pointer->decoded_data ));
    
    printf("%s",cudaGetErrorName(cudaMemcpy(device_memory_pointer->decoded_data, tempdecoded,  sizeof( float ), cudaMemcpyHostToDevice)));
//    printf("%s",cudaGetErrorName(cudaMemcpy(device_memory_pointer->decoded_data, tempdecoded, 10000 * sizeof( float ), cudaMemcpyHostToDevice)));
...
int main()
{
    ...
    struct DF * device_memory_pointer;
    //struct DF  device_memory_pointer;
    corrid_t corrindex = 1;
    printf("\n---- %d ----\n",gpu_init(corrindex, "qwer.txt", device_memory_pointer, 1));
    //gpu_get_phi(freq, file_dec->df->data, 1, device_memory_pointer, 1);
    //gpu_get_phi(freq, file_dec->df->data, 1, device_memory_pointer, 1);
    //gpu_get_phi(freq, file_dec->df->data, 1, device_memory_pointer, 1);
